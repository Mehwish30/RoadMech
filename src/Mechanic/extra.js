...
import { StyleSheet, Text, View ,Button,Image,TouchableHighlight,TouchableOpacity,
       Dimensions,AppRegistry,TextInput,YellowBox,AppState,Alert} from 'react-native';
...
//------------RENDER FUNCTION--------------------//
  ...
  render() {
    ...
    return (
      ...
      <Card style={styles.MainAcceptView} >
        ...
        <Text style={{marginTop:20,fontSize:18,marginLeft:15,color:'#636e72',fontWeight:"bold"}}>{DriverHomeContents.Firstname +""+DriverHomeContents.Lastname}</Text>
        </View>
        ...
        <View style={styles.riderLocationValue}>
             <Text style={{fontSize:14,color:'#42A5F5'}}>{RiderPickUpName}</Text>
             <Text style={{fontSize:14,color:'#42A5F5'}}>{RiderDropUpName}</Text>
        </View>
      ...
      <Card style={styles.MainAcceptView} >
        ...
        <View style={styles.riderLocationValue}>
             <Text style={{fontSize:14,color:'#42A5F5'}}>{D_RiderPickUpName}</Text>
             <Text style={{fontSize:14,color:'#42A5F5'}}>{D_RiderDropUpName}</Text>
        </View>
        <View style={styles.riderPayments}>
        <Text style={{marginTop:20,fontSize:15,fontWeight:'bold'}}>CASH</Text>
        <Image  source={require('../assets/logo.png')} style={{width:20,height:20,borderRadius:10,marginTop:20,marginLeft:7}}  />
  ...
  _StartDriving=async()=>{
   ...
   .then(driverID => 
    ...
    database().ref('Ride_History/'+RiderID+"/").set({
      driverID:driverID,
   ...
   .then(driverID => 
    ...
    database().ref('Ride_History/'+driverID+"/").set({
      riderID:RiderID,
  ...
  _GetDriverRiderDistance=(lat1, lon1, lat2, lon2)=>{
    ...
    //if(userLatitude>0 && userLongitude>0){
      ...
      .then(result => 
        ...
        database().ref(`Drivers/${result}/DriversCurrentLocation/`).set({
    ...
    _getRiderRequestDetails=()=>{
      ...
      .then(result => 
        ...
        database().ref('/Ride_Request/' +result).once('value').then(function(snapshot) {
          ...
          if(snapshot.exists()){
            ...
            if(!RiderID==""){
              ...
              database().ref('/Riders/' +RiderID+'/Details').once('value').then(function(snapshot) {
    ...
    _getRiderAcceptDetails=()=>{
      ...
      .then(result => 
        ...
        database().ref('/Ride_Confirm/' +result).once('value').then(function(snapshot) {
          ...
          if(snapshot.exists()){
            ...
            if(!D_RiderID==""){
              ...
              database().ref('/Riders/' +D_RiderID+'/Details').once('value').then(function(snapshot) {
                DriverHomeContents.D_Firstname = snapshot.child('firstname').val();
  ...
  AsyncStorage.getItem('driverId')
  .then(result => 
    database().ref('/Ride_Request/' +result).remove(),
    database().ref('/Ride_Request/' +RiderID).remove(),
    this.setState({isModalVisible:false})
  ...
  .catch(e => 
   ...
   .then(driverID => 
    ...
    database().ref('Ride_Confirm/'+RiderID).set({
      driverID:driverID,
   ...
   .then(driverID => 
    ...
    database().ref('Ride_Confirm/'+driverID+"/").set({
      riderID:RiderID,
      riderpickname:RiderPickUpName,
      riderdropname:RiderDropUpName,
      riderpickuplatitude:RiderPickUpLatitude,
